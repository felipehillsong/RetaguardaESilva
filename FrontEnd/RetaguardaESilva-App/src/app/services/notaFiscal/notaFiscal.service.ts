...
public GerarPdf(id : number, notaFiscalEmissao:boolean, exclusao:boolean) : Observable<NotaFiscal>{
  return this.http.get<NotaFiscal>(`${this.baseURLGerarPDF}/${id}?empresaId=${this.authService.empresaId()}&notaFiscalEmissao=${notaFiscalEmissao}&exclusao=${exclusao}`).pipe(take(1));
}
